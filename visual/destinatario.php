...
try {
  ...
  $destinatario = $conexion->query($sqlDestinatario);
        ...
        <table class="table table-sm mx-auto  table-hover mt-4">
            ...
            <tbody>
                <?php
                    ...
                    $result =mysqli_query($conexion, "SELECT id_destinatario, nombre_destinatario, apellido_destinatario, telefono_des FROM destinatario");
                    while($fila = mysqli_fetch_assoc($result)) :
                        ?>
                        <tr>
                            <td><?php echo $fila['id_destinatario']; ?></td>
                            <td>
                                <?php if (isset($fila['nombre_destinatario'])) {
                                    echo $fila['nombre_destinatario'];
                                } else {
